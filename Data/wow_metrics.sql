...
SELECT WEEK, 'Week' as Groupi,
ROUND(SUM(in_full)*100 / COUNT(in_full),2) AS 'LIFR%',
ROUND(SUM(in_full)*100 / COUNT(in_full),2) AS 'IF%',
ROUND(SUM(on_time)*100 / COUNT(on_time),2) AS 'OT%',
ROUND(SUM(in_full and on_time)*100 / COUNT(in_full and on_time),2) AS 'OTIF%',
SUM(delivery_qty)*100/SUM(order_qty) AS 'VoFR%'
FROM tmp
GROUP BY WEEK
